...
@Authorization(value = "X-Operation-Token")    }, tags={ "RmsDriver", })
    ...
    @ApiOperation(value = "导入", nickname = "importExcel", notes = "", response = Response.class, authorizations = {
        @Authorization(value = "X-Access-Token"),
@Authorization(value = "X-Operation-Token")    }, tags={ "RmsDriver", })
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "一般返回", response = Response.class) })
    @RequestMapping(value = "rmsDriver/importExcel",
        produces = { "application/json" },
        consumes = { "multipart/form-data" },
        method = RequestMethod.POST)
    default ResponseEntity<Response> importExcel(@ApiParam(value = "file detail") @Valid @RequestPart("file") MultipartFile file) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    return new ResponseEntity<Response>(getObjectMapper().get().readValue("{\r\n  \"result\" : \"result\",\r\n  \"code\" : 0,\r\n  \"message\" : \"message\"\r\n}", Response.class), HttpStatus.NOT_IMPLEMENTED);
                } catch (IOException e) {
                    log.error("Couldn't serialize response for content type application/json", e);
                    return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
                }
